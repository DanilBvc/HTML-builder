...
function starst(){ 
  ...
  /* html copy htmplate  to project (unblock when str replace will be done) */  
  // function copyHTML(to) {
  //   fs.mkdir(path.join(__dirname, to, 'assets'),{recursive:true}, () => {});
  //   fs.readdir(path.join(__dirname), (err, data) => {
  //     if (err) {throw err;}
  //     data.forEach(elem => {
  //       if(elem === 'template.html' ) {
  //         fs.copyFile(path.join(__dirname, elem), path.join(__dirname, to, elem), err => {if (err) throw err;});
  //       }
  //     });
  //   });
  // }
  // copyHTML('project-dist');
  /* */
  let templateTxt = '';
